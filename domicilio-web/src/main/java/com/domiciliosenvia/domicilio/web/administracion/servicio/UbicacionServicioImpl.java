...
public class UbicacionServicioImpl extends AccionesBO implements UbicacionServicioIface {
    ...
    public List<Ubicacion> consultarPorTipo(int tipo, int profundidad) {
        List<Ubicacion> lista = new ArrayList();
        try {
            for (Ubicacion _obj : getUbicacionRemoto().consultarPorTipo(tipo)) {
                if (profundidad > 0) {
                    _obj.setUbicacionPadre(consultarPadre(_obj, profundidad, 1));
                }
                lista.add(_obj);
            }
        } catch (Exception e) {
    ...
    private Ubicacion consultarPadre(Ubicacion hijo, int profFinal, int profActual) {
        Ubicacion padre = new Ubicacion();
        try {
            padre = getUbicacionRemoto().consultarPadre(hijo.getId());
            if (profFinal > profActual) {
                padre.setUbicacionPadre(consultarPadre(padre, profFinal, profActual + 1));
            }
        } catch (Exception e) {
    ...
    public void cargarPadreTipo(UbicacionBean bean) {
        ...
        try {
            ...
            for (Ubicacion _obj : getUbicacionRemoto().consultarPorTipo(bean.getObjeto().getTipo())) {
    ...
    private ArrayList<Ubicacion> consultarPadres(Ubicacion hijo, ArrayList<Ubicacion> listaPadres) {
        Ubicacion padre;
        try {
            padre = getUbicacionRemoto().consultarPadre(hijo.getId());
            if (padre != null) {
                listaPadres.add(padre);
                consultarPadres(padre, listaPadres);
            }
        } catch (Exception e) {
        }
        return listaPadres;
    ...
    public String armarJerarquiaSuperior(Ubicacion ubicacionActual) {
        ArrayList<Ubicacion> listaPadres = new ArrayList<>();
        listaPadres = consultarPadres(ubicacionActual, listaPadres);
    ...
    public ArrayList<Ubicacion> consultarHijos(Ubicacion padre) {
        ArrayList<Ubicacion> listaHijos = new ArrayList<>();
        try {
            for (Ubicacion hijo : getUbicacionRemoto().consultarHijos(padre.getId())) {
                listaHijos.add(hijo);
            }
        } catch (Exception e) {
        }
...
//    @Override
//    public List<Maestro> cargaRegiones() throws Exception {
//        return getMaestroRemoto().consultarPorTipo(MaestroTipo.GN_REGION);
//    }
    @Override
